...
private:
    ...
    auto createInternal(int key, Node * left, Node * right, const int tid);
    auto createLeaf(int key, const int tid);
    void freeSubtree(const int tid, Node * node);
    long getSumOfKeysInSubtree(Node * node);

};

auto ExternalKCASReclaim::createInternal(int key, Node * left, Node * right, const int tid) {
...
auto ExternalKCASReclaim::createLeaf(int key, const int tid) {
    return createInternal(key, NULL, NULL, tid);
}
...
ExternalKCASReclaim::ExternalKCASReclaim(const int _numThreads, const int _minKey, const int _maxKey)
	...
	auto rootLeft = createLeaf(minKey - 1, 0);
    auto rootRight = createLeaf(maxKey + 1, 0);
    root = createInternal(minKey - 1, rootLeft, rootRight, 0);
}
...
bool ExternalKCASReclaim::insertIfAbsent(const int tid, const int & key) {
    ...
    while (true) {
        ...
        auto na = createLeaf(key, tid);
        auto leftChild = (dir <= 0) ? na : ret.n;
        auto rightChild = (dir <= 0) ? ret.n : na;
        auto n1 = createInternal(std::min(key, ret.n->key), leftChild, rightChild, tid);
